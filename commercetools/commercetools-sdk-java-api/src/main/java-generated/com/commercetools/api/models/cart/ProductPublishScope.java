...
public interface ProductPublishScope {
    ...
    */
    ...
    /**
    	<p>Publishes the Prices of the Product (only if the Product is already published). All ProductVariants' Prices in the <em>staged</em> projection are published into the <em>current</em> projection with the same <code>id</code>. Prices in a <em>staged</em> ProductVariant that has no <em>current</em> projection are not published. Prices in a <em>current</em> ProductVariant that has no <em>staged</em> projection are unchanged. The <code>hasStagedChanges</code> <a href="ctp:api:type:ProductCatalogData">flag</a> is updated according to whether the <em>staged</em> and <em>current</em> projections still differ after the prices are published.</p>
